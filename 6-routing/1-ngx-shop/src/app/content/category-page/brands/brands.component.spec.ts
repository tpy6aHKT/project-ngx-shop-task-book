...
describe('[Moдуль 6 - Компонент бренда товаров]', () => {
	...
	xit('тег с селектором .brand-name  должен правильно интерполировать свойство brandName', () => {
		...
		fixture.detectChanges();
		const brandNameElList: DebugElement[] = fixture.debugElement.queryAll(By.css('.brand-name'));
		...
		(component as any).brands.forEach((brand: string, index: number) => {
			expect(brandNameElList[index].nativeElement.textContent.trim()).toEqual(brand);
		});
	});
