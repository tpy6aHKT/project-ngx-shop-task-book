...
export class ReviewPipe implements PipeTransform {
  public transform(countOfReviews: number | undefined): string {
    ...
    let lastNumOfReviews = Number(strNumOfReviews.toString().slice(strNumOfReviews.length - 2));
    ...
    lastNumOfReviews = Number(strNumOfReviews.toString().slice(strNumOfReviews.length - 1));
    ...
    if (lastNumOfReviews > 1 && lastNumOfReviews < 5 || lastNumOfReviews >= 5) {
      return `${numOfReviews} отзывов`;
    }
    return `Нет отзывов`;
  }
